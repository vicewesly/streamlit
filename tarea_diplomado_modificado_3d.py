...
# # 9. Visualización 3D de Variables Clave

st.header('9. Visualización 3D de Variables Clave')

st.markdown("""
Este gráfico tridimensional permite explorar simultáneamente el efecto del precio unitario, 
la cantidad y el total de venta, separados por línea de producto.

**Características mejoradas:**
- Categorización por rangos de precio y cantidad
- Símbolos diferenciados por línea de producto
- Tamaños proporcionales al total de ventas
- Colores categóricos más distintivos
""")

# Crear categorías para hacer el gráfico más categórico
filtered_df_viz = filtered_df.copy()

# Categorizar precios
filtered_df_viz['Categoria_Precio'] = pd.cut(
    filtered_df_viz['Unit price'],
    bins=4,
    labels=['Bajo', 'Medio-Bajo', 'Medio-Alto', 'Alto']
)

# Categorizar cantidades
filtered_df_viz['Categoria_Cantidad'] = pd.cut(
    filtered_df_viz['Quantity'],
    bins=3,
    labels=['Poca', 'Moderada', 'Alta']
)

# Categorizar totales para el tamaño
filtered_df_viz['Categoria_Total'] = pd.cut(
    filtered_df_viz['Total'],
    bins=4,
    labels=['Pequeña', 'Mediana', 'Grande', 'Muy Grande']
)

# Mapear símbolos para cada línea de producto
symbol_map = {
    'Electronic accessories': 'circle',
    'Fashion accessories': 'square',
    'Food and beverages': 'diamond',
    'Health and beauty': 'cross',
    'Home and lifestyle': 'triangle-up',
    'Sports and travel': 'star'
}
...
fig_3d = px.scatter_3d(
    ...
    size='Total',
    hover_data={
        'Categoria_Precio': True,
        'Categoria_Cantidad': True,
        'Categoria_Total': True,
        'Customer type': True,
        'Gender': True
    },
    title='Análisis 3D Categórico: Precio × Cantidad × Total por Línea de Producto',
    labels={
...
# Agregar análisis categórico complementario
col1, col2 = st.columns(2)

with col1:
    st.subheader("Distribución por Categorías de Precio")
    precio_dist = filtered_df_viz.groupby(['Product line', 'Categoria_Precio']).size().reset_index(name='Frecuencia')
    ...
    fig_precio = px.bar(
        ...
        y='Frecuencia',
        color='Product line',
        title='Distribución de Ventas por Rango de Precio',
        labels={'Categoria_Precio': 'Categoría de Precio', 'Frecuencia': 'Número de Ventas'}
    )
    fig_precio.update_layout(height=400)
    st.plotly_chart(fig_precio, use_container_width=True)

with col2:
    st.subheader("Distribución por Categorías de Cantidad")
    cantidad_dist = filtered_df_viz.groupby(['Product line', 'Categoria_Cantidad']).size().reset_index(
        name='Frecuencia')

    fig_cantidad = px.bar(
        cantidad_dist,
        x='Categoria_Cantidad',
        y='Frecuencia',
        color='Product line',
        title='Distribución de Ventas por Rango de Cantidad',
        labels={'Categoria_Cantidad': 'Categoría de Cantidad', 'Frecuencia': 'Número de Ventas'}
    )
    fig_cantidad.update_layout(height=400)
    st.plotly_chart(fig_cantidad, use_container_width=True)

# Tabla resumen categórica
st.subheader("Resumen Categórico por Línea de Producto")
resumen_categorico = filtered_df_viz.groupby('Product line').agg({
    'Categoria_Precio': lambda x: x.mode().iloc[0] if not x.empty else 'N/A',
    'Categoria_Cantidad': lambda x: x.mode().iloc[0] if not x.empty else 'N/A',
    'Categoria_Total': lambda x: x.mode().iloc[0] if not x.empty else 'N/A',
    'Total': ['mean', 'count']
}).round(2)

resumen_categorico.columns = ['Precio Dominante', 'Cantidad Dominante', 'Venta Dominante', 'Promedio Ventas',
                              'Total Transacciones']
st.dataframe(resumen_categorico, use_container_width=True)
...
st.markdown("""
### Hallazgos Clave:
1. Ventas por Línea de Producto: Se evidenvian las líneas de producto que son más rentables,
 esto permite optimizar la asignación de recursos y espacio en tienda para generar más ventas dentro de las sucursales.
...
3. Diferencias dependiendo del tipo de Cliente: Los datos revelan diferencias significativas en el comportamiento de compra entre clientes miembros y clientes normales.

4. Formas de Pago: Se observan preferencias claras en los métodos de pago utilizados, lo que puede informar estrategias de promoción y optimización de procesos.

5. Correlaciones: Las relaciones identificadas entre variables clave como costo y ganancia proporcionan un conocimiento claro que tiene relacioncon la optimización de precios y margen que se genera en la relacion venta - costo.

6. Sello por Sucursal: Cada sucursal muestra un patrón único en la composición de sus ventas por línea de producto, lo que sugiere oportunidades para estrategias localizadas.

Tomando en cuenta las evidencias mencionadas anteriormente, se podría realizar un mejor análisis si se definieran estrategias claves
tanto para el marketing operacional como el de ventas, ya que podríamos segmentar a los clientes por rango etario, y potenciar cientos
productos de acuerdo a ello en sucursales que no cuenten con la misma cantidad de venta. tambien podría evaluarse el sector con la direccion 
de cada sucursal e ir con una estrategia potente para mejorar o pontenciar ciertos productos que se requieran vender. 

### Recomendaciones:

Recomendaciones Estratégicas
1. Optimización del Inventario: Reasignar espacio y stock priorizando las líneas de productos más rentables, especialmente en sucursales donde no se están aprovechando al máximo.

2. Estrategias de Marketing Segmentado: Definir campañas específicas por rango etario o tipo de cliente, potenciando aquellos productos con mejor calificación o bajo rendimiento en ciertos segmentos.

3. Enfoque Geográfico por Sucursal: Analizar el entorno socioeconómico y demográfico de cada sucursal para aplicar acciones más efectivas: por ejemplo, introducir productos de mayor rotación en zonas residenciales o reforzar promociones en sectores comerciales.

4. Promociones y Fidelización: Diseñar estrategias de fidelización para clientes frecuentes o miembros, como descuentos exclusivos, programas de puntos o contenido personalizado.

5. Promover Métodos de Pago Específicos: Fomentar el uso de medios de pago más eficientes o rentables para la empresa a través de descuentos u ofertas exclusivas.

6. Monitoreo Continuo de KPIs: Implementar dashboards operacionales por sucursal para monitorear en tiempo real los principales KPIs y ajustar rápidamente las decisiones comerciales.
""")
